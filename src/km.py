...
class KeplerMapper(object):
  ...
  def fit_transform(self, X, projection="sum", scaler=preprocessing.MinMaxScaler()):
    ...
    if isinstance(projection, str):
      if self.verbose > 0:
        print("Projecting data using: %s"%(projection))
      # Stats lenses
    ...
    # Detect if projection is a list (with dimension indices)
      ...
      if self.verbose > 0:
        print("Projecting data using: %s"%(str(projection)))
      X = X[:,np.array(projection)]
    
    # Detect if projection is a class (for scikit-learn)
    elif str(type(projection))[1:6] == "class": #TODO: de-ugly-fy
      reducer = projection
      if self.verbose > 0:
        try:    
    ...
    if scaler is not None:
      if self.verbose > 0:
        print("Scaling with: %s"%str(scaler))
  ...
  def map(self, projected_X, inverse_X=None, clusterer=cluster.DBSCAN(eps=0.5,min_samples=3), nr_cubes=10, overlap_perc=0.1):
    ...
    if self.verbose > 0:
      print("Mapping on data shaped %s using dimensions"%(str(projected_X.shape)))
    ...
    if self.verbose > 0:
      total_cubes = len(cube_coordinates_all(nr_cubes,projected_X.shape[1]-1))
    ...
    for i, coor in enumerate(cube_coordinates_all(nr_cubes,di.shape[0])):
      ...
      if hypercube.shape[0] > 0:
        ...
        if self.verbose > 1:
          print("Found %s clusters in cube_%s"%(np.unique(clusterer.labels_[clusterer.labels_ > -1]).shape[0],i))
    ...
    if self.verbose > 0:
      ...
      print("Created %s edges and %s nodes in %s."%(nr_links,len(nodes),str(datetime.now()-start)))
  ...
  def visualize(self, complex, color_function="", path_html="mapper_visualization_output.html", title="My Data", 
          ...
          height_html=0, show_tooltips=True, show_title=True, show_meta=True, bg_color_css="#111"):
    ...
    * {margin: 0; padding: 0;}
    html { height: 100%%;}
    body {height: 100%%; font: 100 16px Roboto, Sans-serif;}
    ...
    #holder {background: %s; position: relative; width: %s; height: %s; display: block;}
    ...
    var divs = d3.select('#holder').append('div')
      ...
      node.attr("cx", function(d) { return d.x; })
        ...
        .attr('style', function(d) { return 'width: ' + (d.group * 2) + 'px; height: ' + (d.group * 2) + 'px; ' + 'left: '+(d.x-(d.group))+'px; ' + 'top: '+(d.y-(d.group))+'px; background: '+color(d.color)+'; box-shadow: 0px 0px 3px #111;'})
        ;
      });
    </script>"""%(title, bg_color_css, width_css, height_css, title_display, meta_display, tooltips_display, title,complex["meta"],self.nr_cubes,self.overlap_perc*100,color_function,complex["meta"],str(self.clusterer),str(self.scaler),width_js,height_js,graph_charge,graph_link_distance,graph_gravity,json.dumps(json_s))
      outfile.write(html.encode("utf-8"))
    if self.verbose > 0:
      print("Wrote d3.js graph to '%s'"%path_html)